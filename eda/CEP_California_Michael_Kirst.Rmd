...
district_df %>% ungroup() %>% summarise(`Mean Proposed ISP` = weighted.mean(ISP_proposed,proposed_eligibility,na.rm=TRUE)) %>% knitr::kable()
district_df %>% ungroup() %>% summarise(`Mean Current ISP` = weighted.mean(ISP_current,current_eligibility,na.rm=TRUE)) %>% knitr::kable()
