...
## Introduction
Currently, The vast majority of schools do not actively participate in any type of federal provisions for universal meal service. Of those that do, most participate in the Community Eligibility Provision. Schools are able to participate in CEP if at least 40% of their students are identified as eligible for universal meal service via SNAP, TANF, Calfresh and other programs.  Schools that participate in the program must keep a daily total of meal service and not charge students for meals. 
```{r,echo=FALSE}
df_clean %>%
...
The figures below illustrates current the distribution of schools based on whether or not they qualify for the program on an individual basis. The vertical lines correspond to the 40% threshold at which schools become eligible to participate on the program on an independent basis and 62.5% where schools receive the maximal federal reimbursement rate. Assuming schools must participate in the program independently, over a half-million students could be participating in the program.
```{r,echo=FALSE}
df
...
Schools tend to participate with increasing probability as the number of students who as the identified student percentage increases. We find not only are there is increasing probability of enrollment, non-parametric tests suggest that there are sharp discontinuities in participation at the 40% and 62.5% thresholds where the reimbursement policy respectivelykick in and level off. 
...
district_df %>% ungroup() %>% summarise(mean=weighted.mean(ISP_proposed,proposed_eligibility,na.rm=TRUE))


district_df %>% ungroup() %>% summarise(mean=weighted.mean(ISP_current,current_eligibility,na.rm=TRUE))
...
cep_max_schools %>%
  ...
  district_df  %>%  ungroup() %>% filter(district_gain>0) %>% summarise(district_gain=sum(district_gain))
...
Moreover, these results also suggest that there are likely non-trivial gains that can occur by using more advanced optimization techniques. Currently, all of these analyses used a simple grid search algorithm that works well but are suboptimal. In general
```{r,echo=FALSE}
