...
district_baseline %>% left_join(cep_eligible_schools,by=c("state","school_district_local_education_agency_or_lea"="lea")) %>%
  ...
  select(pct_gain,raw_gain,max_enrolled,everything()) %>% 
  arrange(desc(raw_gain))  %>% 
  group_by() %>% filter(state=="Colorado")
...
district_baseline %>% left_join(cep_eligible_schools,by=c("state","school_district_local_education_agency_or_lea"="lea")) %>%
  ...
  arrange(desc(raw_gain))  %>% 
  group_by(state) %>%
  summarise_at(c("receives_meals","raw_gain","max_enrolled","total_enrollment"),sum,na.rm=TRUE) %>%
  arrange(raw_gain) %>% 
  mutate(
